namespace WebApplication2;

public class DataTa
{
    public string? cabinet { get; set; }
    public string? teacher1 { get; set; }
    public string? droup1 { get; set; }
    public string? teacher2 { get; set; }
    public string? droup2 { get; set; }
    public string? teacher3 { get; set; }
    public string? droup3 { get; set; }
    public string? teacher4 { get; set; }
    public string? droup4 { get; set; }
    public string? teacher5 { get; set; }
    public string? droup5 { get; set; }
    public string? teacher6 { get; set; }
    public string? droup6 { get; set; }
    public string? teacher7 { get; set; }
    public string? droup7 { get; set; }
    public string? day { get; set; }

}